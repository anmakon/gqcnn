...
class Show_DexNet_Data():
    ...
    def _get_label(self):
        robust = np.load(self.data_path + 'robust_ferrari_canny_' + self._gen_filepointer() + '.npz')['arr_0'][
            self.array_pointer]
        if robust >= 0.002:
            return 1
        else:
    ...
    def _get_obj_id(self):
        return np.load(self.data_path + 'object_labels_' + self._gen_filepointer() + '.npz')['arr_0'][
            self.array_pointer]
    ...
    def _visualise(self):
        ...
        depth_im_table = np.load(self.data_path + 'depth_ims_tf_table_' + self._gen_filepointer() + '.npz')['arr_0'][
            self.array_pointer]
    ...
    def _save_depth_hist(self, depths, depth_im):
        ...
        with open(self.savepath + 'DexNet_' + object_des + '_depth_' + self._gen_filepointer() + '_' + str(
                array_pointer) + '.csv', 'w', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter=',')
            ...
            for row in table:
                writer.writerow(row)
        depth_im.save(self.savepath + 'DexNet_' + object_des + '_' + self._gen_filepointer() + '_' + str(
            self.array_pointer) + '.png')
        return None
